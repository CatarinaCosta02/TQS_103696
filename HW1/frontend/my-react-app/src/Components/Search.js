...
const Search = () => {
    ...
    const submitCity = async () => {
        ...
        if (city) {
            setLoading(true);
            await fetch(`http://localhost:8080/api/current/${city}`)
                .then((res) => {
                    if (res.status === 200)
                        return res.json();
                })
    ...
    return (
        ...
        <table className="table w-full">
          ...
          <tbody>
            <tr>
              <td>
                <span>{data ? data.data[0].aqi : "-"}</span>
              </td>
              <td>
                <span>{data ? data.data[0].o3 : "-"}</span>
              </td>
              <td>
                <span>{data ? data.data[0].so2 : "-"}</span>
              </td>
                <td>
                <span>{data ? data.data[0].no2 : "-"}</span>
                </td>
                <td>
                <span>{data ? data.data[0].pm10 : "-"}</span>
                </td>
                <td>
                <span>{data ? data.data[0].pm25 : "-"}</span>
                </td>
            </tr>
          </tbody>
        </table>
        ...
        <table className="table w-full">
          <thead>
            <tr>
              ...
              <th>Mold Level</th>
              <th>Predominant Pollen Type</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td>
                <span>{data ? data.data[0].pollen_level_tree : "-"}</span>
              </td>
                <td>
                <span>{data ? data.data[0].pollen_level_grass : "-"}</span>
                </td>
                <td>
                <span>{data ? data.data[0].pollen_level_weed : "-"}</span>
                </td>
                <td>
                <span>{data ? data.data[0].mold_level : "-"}</span>
                </td>
                <td>
                <span>{data ? data.data[0].predominant_pollen_type : "-"}</span>
                </td>
            </tr>
          </tbody>
        </table>
